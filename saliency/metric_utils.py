'''
Modified from 'https://github.com/tarunsharma1/saliency_metrics/blob/master/salience_metrics.py
...
if __name__ == '__main__':
    img_path = '/home/qiang/QiangLi/Python_Utils_Functional/FixaTons/MIT1003'

    # this is just the name its not actually discretised (binary)
    gt = cv2.imread(os.path.join(img_path, 'SALIENCY_MAPS', 'i1508828.jpg'),0)
    gt = cv2.resize(gt, (682, 682))
    plt.figure(figsize=(10,10))
    plt.imshow(gt)
    plt.axis('off')
    plt.show()
    
    s_map = cv2.imread(os.path.join(img_path, 'SALIENCY_MAPS_QL', 'i1508828.jpeg'),0)
    s_map_norm = normalize_map(s_map)
    plt.figure(figsize=(10,10))
    plt.imshow(s_map_norm)
    plt.axis('off')
    plt.show()
