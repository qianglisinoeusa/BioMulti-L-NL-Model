...
if size(scanpath,1)>0
    ...
    x=scanpath(:,2);
    hold on
    plot(y(1),x(1),'--go','MarkerSize',markersize(1),'LineWidth',2,'MarkerFaceColor',[0 1 0]);
    text(y(1)-round(markersize(1)*0.5), x(1), num2str(1),'Color','red','FontSize',markersize(1)*0.5);
    for g=2:size(scanpath,1)
        plot([y(g-1),y(g)],[x(g-1),x(g)],'g--','MarkerSize',1,'LineWidth',2);
        plot(y(g),x(g),'--go','MarkerSize',markersize(g),'LineWidth',2,'MarkerFaceColor',[0 1 0]);
