...
function [score,tp_s,fp_s] = AUC_Borji(saliencyMap, fixationMap, Nsplits, stepSize, toPlot)
...
if nargin < 3, if sum(sum(fixationMap)) > 100, Nsplits = 100; else Nsplits = sum(sum(fixationMap)); end; end
