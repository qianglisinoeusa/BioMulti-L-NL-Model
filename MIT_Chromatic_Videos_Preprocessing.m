...
for i=1:numel(files_L) 
    ...
    %show_fft3_color( abs(FTY).^0.25,75,20,4),axis square
    %show_project_fft3(abs(FTY).^0.25,75,20,5),axis square
    %set(4,'color',[1 1 1])  
    %view_fourier = [-113 24];
    %view(view_fourier)
    %title(files_L{i}(1:4));
...
%     for ii=1:nf
% 
%         fotog=sacafot(fx,m(1),m(1),ii);
%         Fx(:,:,ii)=fotog;
%         fotog=sacafot(fy,m(1),m(1),ii);
%         Fy(:,:,ii)=fotog;
%         fotog=sacafot(ft,m(1),m(1),ii);
%         Ft(:,:,ii)=fotog;
%         fotog=sacafot(tf,m(1),m(1),ii);
%         TF(:,:,ii)=fotog;
%     end
%     %[max(Fx(:))  max(Fy(:)) max(Ft(:)) max(TF(:))]
% 
%     
%     Yf = then2now(FTY, 268);
%     ffs = reshape(Yf, [268 268 3 30]);
%     ffs = ffs(:,:,1,:);
%     %implay(abs(fftshift(ffs)).^0.6/255);
%     %pause(0.3)
%     %close all
%     sp = max(max(max(abs(fftshift(ffs)).^0.1)));
%     speed_curve = [speed_curve sp];
%    
%     %GLCM2 = graycomatrix(now2then(real(ffs)),'Offset',[2 0;0 2]);
%     %stats = graycoprops(GLCM2,{'energy'});
%     tfxy=proytf3(tf,1);
%     te=maxi(tfxy);
%     texture_curve = [texture_curve te];
